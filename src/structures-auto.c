...
int vk_db_insert_message (struct message *r) {
  struct message *old = vk_db_lookup_message (r->id);
  if (old && !cmp_message (r, old)) { return 0; }
  char *q;
  if (!old) {
    q = sqlite3_mprintf ("INSERT INTO messages (id,uid,date,read_state,out,title,body,chat_id,deleted,emoji) VALUES (%d,%d,%d,%d,%d,%Q,%Q,%d,%d,%d);",r->id,r->uid,r->date,r->read_state,r->out,r->title,r->body,r->chat_id,r->deleted,r->emoji);
  } else {
    q = sqlite3_mprintf ("UPDATE messages SET id=%d,uid=%d,date=%d,read_state=%d,out=%d,title=%Q,body=%Q,chat_id=%d,deleted=%d,emoji=%d WHERE id=%d;",r->id,r->uid,r->date,r->read_state,r->out,r->title,r->body,r->chat_id,r->deleted,r->emoji,r->id);
  }
  assert (q);
  if (verbosity >= 4) {
    fprintf (stderr, "%s\n", q);
...
struct message *vk_parse_message (json_t *J) {
  ...
  r->id = parse_integer_value1 (J,"mid");
